...
export default async function AdminPage() {
  ...
  const memberExpenseSummary = expenses.reduce<Record<string, { submittedByAdmin: number; totalUsed: number }>>(
    (acc: Record<string, { submittedByAdmin: number; totalUsed: number }>, expense: Expense) => {
      ...
      // Total used by member: all submitted expenses
      acc[memberId].totalUsed += expense.amount

      // Submitted by admin: only expenses that are marked paid
      if (expense.status === "PAID") {
        acc[memberId].submittedByAdmin += expense.amount
      }
            ...
            expenses.map((expense: Expense) => (
              <div key={expense.id} className="rounded-lg border border-gray-200 bg-white p-4 space-y-3">
                {(() => {
                  ...
                  const remainingExpense = submittedExpense - totalExpense
                ...
                <div className="grid grid-cols-2 gap-3 text-sm">
                  ...
                  <div>
                    <p className="text-gray-500">Submitted Expense</p>
                    <p className="font-medium text-gray-900">{formatCurrency(submittedExpense)}</p>
                  </div>
                  <div>
                    <p className="text-gray-500">Total Expense</p>
                    <p className="font-medium text-gray-900">{formatCurrency(totalExpense)}</p>
                  </div>
                  <div>
                    <p className="text-gray-500">Remaining</p>
                    <p className={`font-medium ${remainingExpense >= 0 ? "text-green-700" : "text-red-700"}`}>
          ...
          <table className="min-w-full text-sm">
            <thead className="bg-gray-50 text-left text-gray-600">
              <tr>
                ...
                <th className="px-4 py-3 font-semibold">Submitted Expense</th>
              ...
              {expenses.length === 0 ? (
                ...
                expenses.map((expense: Expense) => {
                  ...
                  <tr key={expense.id} className="border-t border-gray-100">
                    ...
                    <td className="px-4 py-3 text-gray-900">{formatCurrency(submittedExpense)}</td>
                    <td className="px-4 py-3 text-gray-900">{formatCurrency(totalExpense)}</td>
                    <td className={`px-4 py-3 ${remainingExpense >= 0 ? "text-green-700" : "text-red-700"}`}>
                      {formatCurrency(remainingExpense)}
                    </td>
